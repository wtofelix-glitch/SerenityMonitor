...
class TestGetPositionSignal:
    ...
    def test_holding_high_profit_high_score_strong_hold(self):
        """持仓中，高浮盈+高评分 → STRONG_HOLD"""
        assert get_position_signal(70, 10.0, True) == "STRONG_HOLD"

    def test_holding_high_profit_good_score_consider_add(self):
        """持仓中，高浮盈+次高评分 → CONSIDER_ADD"""
        assert get_position_signal(58, 8.0, True) == "CONSIDER_ADD"

    def test_holding_low_score_weak_hold(self):
        """持仓中，低评分 → WEAK_HOLD"""
    ...
    def test_not_holding_returns_base_signal(self):
        """非持仓标的使用基础信号"""
    ...
    def test_boundary_48_is_hold_not_weak_hold(self):
        """边界：评分48对于持仓是HOLD（不低于48）"""
        assert get_position_signal(48.0, 0, True) != "WEAK_HOLD"

    def test_boundary_47_9_is_weak_hold(self):
        """边界：评分47.9对于持仓是WEAK_HOLD"""
        assert get_position_signal(47.9, 0, True) == "WEAK_HOLD"

    def test_buy_upgraded_to_strong_hold_for_positions(self):
    ...
    def test_sell_not_blocked_by_position_buffers(self):
...
class TestSignalLevelsConfig:
    ...
    def test_trade_signal_levels_ordered_by_score(self):
        ...
        trade_levels = {k: v for k, v in SIGNAL_LEVELS.items()
                        if k not in ("STRONG_HOLD", "WEAK_HOLD", "CONSIDER_ADD")}
